Dado('que meu perfil de anunciante é {string} e {string}') do |email, password|
 @email_anunaciante = email
 @pass_anunciante   = password
end

Dado('que eu tenho o seguinte equipamento cadastrado:') do |table|
  user_id =  SessionsServices.new.get_user_id(@email_anunaciante, @pass_anunciante)
  ...
  MongoDB.new.remove_equipo(@equipo[:name], @email_anunaciante)
...
Dado('acesso meu Dashboard') do
  @login_page.open
  @login_page.with(@email_anunaciante, @pass_anunciante)
...
Quando('{string} e {string} solicita a locação desse equipo') do |email, password|
  user_id =  SessionsServices.new.get_user_id(email, password)
...
Então('deve ver os links: {string} e {string} no pedido') do |button_accept, button_reject|
